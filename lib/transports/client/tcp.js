...
// connection
const connect = function connect(toReconnect) {
  this.logger('onClose.reconnect - old con is: ' + (this.con && this.con.random))
  const oldPort = this.con && this.con.random
  ...
  this.con = net.connect(this.tcpConfig, () => {
    ...
    if(toReconnect) {
      ...
      // is called.
      process.nextTick(() => {
        Object.keys(oldReqs).forEach((key) => {
...
class TcpTransport extends EventEmitter {
  constructor(tcpConfig, config) {
    // Shim to support old-style call TODO: Eliminate with 1.0.0
